...
`theta` and [`girth`](https://github.com/eribean/girth) fit the **same** models by
...
![theta vs girth speedup, 1PL and 2PL](https://raw.githubusercontent.com/vuciv/theta/main/bench/benchmark.png)

> theta-over-girth speedup (`girth time ÷ theta time`, higher = theta faster).
> CPU (Apple M-series), `Q = 61`, warm (JIT already compiled). On 2PL `theta` is
> 15–50× faster; on the lightweight 1PL/Rasch fit it is ~even on tiny problems
> and pulls ahead with scale. Reproduce with
> `uv run --group bench python bench/compare.py`.

| model | size (persons × items) | theta | girth | speedup | converged | agreement (a, b) |
|:--|--|--:|--:|--:|:--:|--:|
| 2PL | 1,000 × 50   | **61 ms**  | 923 ms | **15×**  | yes | 1.000, 1.000 |
| 2PL | 20,000 × 100 | **0.44 s** | 22.9 s | **52×**  | yes | 1.000, 1.000 |
| 2PL | 50,000 × 100 | **1.0 s**  | 47.6 s | **47×**  | yes | 1.000, 1.000 |
| 1PL | 1,000 × 50   | 80 ms      | **58 ms** | 0.7×  | yes | 1.000, 1.000 |
| 1PL | 20,000 × 100 | **0.46 s** | 1.5 s  | **3.2×** | yes | 0.999, 1.000 |
| 1PL | 50,000 × 100 | **0.89 s** | 4.7 s  | **5.3×** | yes | 0.999, 1.000 |
